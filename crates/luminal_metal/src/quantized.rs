...
kernel void matvec(
    ...
    {type_name} sumf[num_rows] = {{ 0.{type_suffix} }};
    ...
    for (int ib = ix; ib < num_quants_per_row; ib += 8) {{ // ib: current column position
        ...
        for (int row = 0; row < 4; ++row) {{
            ...
            {type_name} sumq = 0.{type_suffix}; // Partial sum
    ...
    for (int row = 0; row < num_rows; ++row) {{
        const {type_name} tot = simd_sum(sumf[row]);
...
kernel void metal_gather(device half *inp [[buffer(0)]], device block_q8_0 *weights [[buffer(1)]], device {type_name} *out [[buffer(2)]], device int& n_embeddings [[buffer(3)]], device int& embedding_dim [[buffer(4)]], uint2 idx [[thread_position_in_grid]]) {{
...
impl<T: MetalFloat> Operator for QuantizedGather<T> {
    fn process(&mut self, tensors: Vec<(InputTensor, ShapeTracker)>) -> Vec<Tensor> {
        autoreleasepool(|| {
            // Setup buffers
            let indexes = tensors[0].0.borrowed().downcast_ref::<Vec<f16>>().unwrap();
            let index_buffer = self.device.new_buffer_with_data(
                unsafe { std::mem::transmute(indexes.as_ptr()) },
                (indexes.len() * std::mem::size_of::<f16>()) as u64,
                MTLResourceOptions::StorageModeShared,
            );
